...
namespace SmartInstaller.Util {
    public class IniFile {
        ...
        public bool ExistKey(string resItemKey) {
            for (int i = 0; i < lstIri.Count; i++) {
                if (lstIri[i].key.Equals(resItemKey, StringComparison.OrdinalIgnoreCase)) {
                    return true;
                }
            }
            return false;
        ...
        public void setValue(string strKey, string strValue) {
            IniResItem resItem = new IniResItem {
                key = strKey,
                value = strValue
            };
            updateInIResItem(resItem);
        }
        ...
        private void readIniFile() {
            if (this.iniFormat == 0) {
                ...
                UtilMessage.ShowError("不支持的ini格式类型。");
            }
        }
        private void readIniFile0() {
            ...
            try {
                ...
                while (sr.Peek() >= 0) {
                    ...
                    if (strLine.Length < 4 || strLine.Substring(0, 4).Equals("rem ", StringComparison.OrdinalIgnoreCase)) {
        ...
        private void readIniFile1() {
            ...
            try {
                if (!File.Exists(iniFileName)) {
                    UtilFile.WriteFile(iniFileName, "");
                }
                if (!File.Exists(iniFileName)) {
                    throw new Exception("配置文件 " + iniFileName + " 不存在，请检查。");
                }
                    ...
                    if (strLine.Length < 4) {
                        ...
                        for (int i = 0; i < arrResItem.Length; i++) {
                            ...
                            if (strKey.Equals("key", StringComparison.CurrentCultureIgnoreCase)) {
                                ...
                                throw new Exception("未知的 ini 配置项名称：" + strKey + "，请检查。");
                            }
